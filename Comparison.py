...
def createGraph(title, xLabel, yLabel, v1, v1Label, v2, v2Label):
    g = plt.figure()
    plt.plot(range(1, len(v1) + 1), v2, 'r', label=f'{v2Label}')
    plt.plot(range(1, len(v1) + 1), v1, 'g', label=f'{v1Label}')
    plt.title(f"{title}")
    plt.xlabel(f"{xLabel}")
    plt.ylabel(f"{yLabel}")
    plt.legend()
...
def getAvgUtilitiesAndTimes(targetNum, avgCount=10):
    ...
    for _ in range(avgCount):
        ...
        # ================================
        # BP Model with extra empty target
        # ================================
        for m in defenders:
            dRewards[m].append(0)
            dPenalties[m].append(0)
            dCosts[m].append(0)
        placements = getPlacements(defenders, targetNum + 1)
        omegaKeys = getOmegaKeys(aTypes, placements, attackerActions)
        baselineStart = getTime()
        model2 = Model('BayesianPersuasionSolver')
        w = model2.continuous_var_dict(keys=omegaKeys, lb=0, ub=1, name="w")
        objectiveFunction = sum([q[lam] * sum([w[(s,a,lam)] * defenderSocialUtility(s,a,defenders, dCosts, dPenalties) for s in placements for a in attackerActions]) for lam in aTypes])
        model2.add_constraints([sum([w[(s,a,lam)] * aUtility(s,a,lam, aPenalties, aRewards) for s in placements]) >= sum([w[(s,a,lam)] * aUtility(s,b,lam, aPenalties, aRewards) for s in placements]) for a in attackerActions for b in attackerActions if a != b for lam in aTypes], names=[f"c att {lam} suggested {a}, but goes to {b}" for a in attackerActions for b in attackerActions if a != b for lam in aTypes])
        model2.add_constraints([sum([q[lam] * sum([w[dm,a,lam] * utilityM(d,dm,a,m, dRewards, dPenalties, dCosts) for a in attackerActions for dm in placements if dm[m] == d])]) >= \
                               sum([q[lam] * sum([w[dm,a,lam] * utilityM(e,dm,a,m, dRewards, dPenalties, dCosts) for a in attackerActions for dm in placements if dm[m] == d])]) \
...
for targetNum in range(targetFloor, targetCeiling + 1):
    print(f"targetNum {targetNum} of {targetCeiling}")
